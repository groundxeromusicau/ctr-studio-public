...
async function run() {
  ...
  const health = await request('/api/health'); expect(health.status === 200, 'health endpoint failed'); const identity = JSON.parse(health.body); expect(identity.product === 'CTR Studio Open', 'wrong product identity'); expect(identity.version === '2.0.0-alpha.2', 'wrong Open release version');
  const home = await request('/'); expect(home.status === 200, 'home page failed'); expect(home.body.includes('<title>CTR Studio Open — Build Your Show. Go Live.</title>'), 'Open website title missing'); expect(home.body.includes('OPEN · VERSION 2.0 ALPHA 2'), 'Open version badge missing');
  ...
  const studio = await request('/studio.html'); expect(studio.status === 200, 'studio page failed'); expect(studio.body.includes('<title>CTR Studio Open</title>'), 'Open studio title missing'); expect(studio.body.includes('V2.0 · ALPHA 2'), 'Open build version missing');
